...
<ol class="breadcrumb">
    ...
    <li><a href="{{ url('/category-products') }}">{{ __('product::categories.category') }}</a></li>
...
<div class="box">
    <div class="box-header">
        ...
        <div class="box-tools">
            <a href="{{ url('/category-products') }}" class="btn btn-warning btn-sm"><i class="fa fa-arrow-left"
                    aria-hidden="true"></i> <span class="hidden-xs">{{ trans('message.lists') }}</span></a>
            @can('CategoryProductController@update')
            <a href="{{ url('/category-products/' . $category->id . '/edit') }}" class="btn btn-primary btn-sm"><i
    ...
    <div class="box-body table-responsive no-padding">
        <table class="table table-striped">
            <tbody>
                <tr>
                    <th>{{ trans('product::categories.name') }}</th>
                    <td>{{ $category->name }} </td>
                </tr>
                <tr>
                    <th>{{ trans('product::categories.url') }}</th>
                    <td><a target="_blank" href="{{ url('tin-tuc/'.$category->slug) }}">{{ url('tin-tuc/'.$category->slug) }}</a></td>
                </tr>
                <tr>
                    <th>{{ trans('product::categories.image') }}</th>
                    <td>
                        @if(!empty($category->image))
                        <img width="100" src="{{ $category->image }}" alt="anh" />
                        @endif
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    <th>{{ trans('product::categories.updated_at') }}</th>
